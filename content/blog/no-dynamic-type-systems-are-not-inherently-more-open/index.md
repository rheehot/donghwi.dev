...
주장은 간단합니다. 정적 타입 시스템에서는 데이터의 형태를 꼭 미리 선언해야 하지만, 동적 타입 시스템에서는, 타입이 말 그대로 동적이라는 것입니다. 설명이 필요 없을 정도로 자명합니다. 너무나도 자명하고 매력적이어서, Rich Hickey[^역주2]는 사실상 이 문장을 가지고 강연 커리어를 쌓았다고 보아도 무방할 정도입니다. 유일한 문제는 이 주장이 틀렸다는 것입니다.
