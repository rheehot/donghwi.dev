...
"동적 타입 시스템이 세상을 모델링 하는 데에 (정적 타입 시스템보다) 더욱 적합하다" 라는 미신이 타입 시스템에 대한 논의를 계속 흐리고 있습니다. 이런 종류의 주장은 보통 이렇게 전개됩니다.

> 정적 타이핑의 목적은 최대한 많은 것들을 못박아두는 것에 있지만, 실제 세계에서는 이것이 실용적이지 않습니다. 실제 시스템들은 서로 느슨하게 연결되어야 하며, 데이터의 표현 방식에 대해서는 신경을 최대한 쓰지 말아야 합니다. 따라서 결국 넓게 보았을 땐, 동적 타입 시스템이 더 견고한 시스템입니다.

꽤 설득력 있게 들리지만, 사실이 아닙니다. 결점은 바로 "잘못된 전제"에 있습니다. 정적 타이핑의 목적은 "모든 세상을 분류"하거나 "시스템에 존재하는 모든 값들을 못박아두는 것"이 아닙니다. 정적 타이핑은 각 요소가 들어오는 입력의 구조에 대해서 얼만큼 알고, 반대로 또 얼마나 몰라야 하는 지를 결정할 수 있도록 해주는 것입니다. 실제로 정적 타입 시스템은 애플리케이션이 필요 이상의 가정을 하는 것을 막아주기 때문에, 일부만 알려져 있는 데이터 구조를 처리하는 데에 굉장히 뛰어납니다.

## 두 가지의 오류

저는 이 주제의 글을 예전부터 쓰고 싶어했습니다. 글을 쓰기로 결심하게 된 결정적인 계기는 바로 [제 전 포스트](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)에 있는 잘못된 댓글들입니다. 두 개의 댓글이 제 눈에 들어왔는데, 하나는 [/r/programming](https://www.reddit.com/r/programming)에 달린 댓글입니다.
