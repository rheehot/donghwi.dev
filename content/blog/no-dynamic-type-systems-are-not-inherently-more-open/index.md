...
동적 타입 시스템이 개방적인 세계를 모델링 하는 데에 있어 정적 타입 시스템보다 본질적으로 더 낫다는 미신이 타입 시스템에 대한 논의를 계속 흐리고 있습니다. 이런 종류의 주장들은 보통 "정적 타이핑의 목적은 최대한 많은 것들을 못박아두는 것에 있지만, 실제 세계에서는 이것이 실용적이지 않다. 실제 시스템들은 서로 느슨하게 연결되어야 하며, 데이터의 표현 방식에 대해서는 신경을 최대한 쓰지 말아야 한다. 따라서 결국 넓게 보았을 땐, 동적 타입 시스템이 더 견고한 시스템이다."와 같은 방식으로 전개됩니다.
